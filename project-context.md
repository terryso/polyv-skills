...
| 类别 | 选择 |
|------|------|
| 语言 | JavaScript (ES2022+) / TypeScript (测试) |
| 运行时 | Node.js 18+ (推荐 23+) |
| 单元测试 | Mocha |
| E2E 测试 | Playwright |
| 包管理 | npm |
| 构建工具 | 无（无需编译） |
...
| 类型 | 框架 | 文件格式 |
|------|------|----------|
| 单元测试 | Mocha | `*.test.js` (CommonJS) |
| E2E 测试 | Playwright | `*.spec.ts` (TypeScript) |

### 测试文件命名
- `tests/unit/*.test.js` - 单元测试 (Mocha)
- `tests/e2e/*.spec.ts` - E2E 测试 (Playwright)
